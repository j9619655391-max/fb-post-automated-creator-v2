"""Apply Alembic migrations and seed optional local development data."""
import subprocess
import sys
...
def create_sample_data() -> None:
    """Create sample users/content for local development only."""
    db = SessionLocal()
    try:
        ...
        db.rollback()
        raise RuntimeError(f"Error creating sample data: {exc}") from exc
    finally:
        db.close()
